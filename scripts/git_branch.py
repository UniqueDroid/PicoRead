...
PICOREAD_VERSION for the default (dev) environment.

Results in a version string like:  1.1.0-dev-feat-kosync-xpath-05c6cf8
Release environments are unaffected; they set PICOREAD_VERSION in the ini.
"""
...
def get_base_version(project_dir):
    ...
    if not config.has_option('picoread', 'version'):
        warn('No [picoread] version in platformio.ini; base version will be "0.0.0"')
        return '0.0.0'
    return config.get('picoread', 'version')
...
def inject_version(env):
    ...
    env.Append(CPPDEFINES=[('PICOREAD_VERSION', f'\\"{version_string}\\"')])
    print(f'PicoRead build version: {version_string}')
