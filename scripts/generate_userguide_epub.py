...
OUTPUT_EPUB = ROOT / "PicoRead_User_Guide.epub"
...
def split_chapters(html: str):
    ...
    if not positions:
        return [('intro', 'PicoRead User Guide', html)]
...
def build_epub():
    ...
    book = epub.EpubBook()
    book.set_identifier('picoread-user-guide-v1')
    book.set_title('PicoRead User Guide')
    book.set_language('en')
    book.add_author('PicoRead Project')
    ...
    # Full-size cover image (480×800) — used by PicoRead for home-screen thumbnail.
    ...
    # EPUB 2 cover declaration — PicoRead's Tier 1 OPF lookup
    book.add_metadata('OPF', 'meta', '', {'name': 'cover', 'content': 'cover-image'})

    # Cover page XHTML — first readable page in the spine
    cover_page = epub.EpubHtml(title='PicoRead User Guide', file_name='cover.xhtml', lang='en')
    cover_page.content = make_xhtml(
        'PicoRead User Guide',
        '<div class="cover-page">'
        '<img src="images/cover.png" alt="PicoRead User Guide cover"/>'
        '</div>',
    )
    ...
    # nav is excluded from the spine — PicoRead locates it via properties="nav" in
