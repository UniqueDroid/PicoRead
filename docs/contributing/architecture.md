...
PicoRead is firmware for the Xteink X4 (unaffiliated with Xteink), built with PlatformIO targeting the ESP32-C3 microcontroller.
...
- `src/PicoReadSettings.h` (`SETTINGS`): user preferences and behavior flags
- `src/PicoReadState.h` (`APP_STATE`): runtime/session state such as current book and sleep context
...
Network file transfer is controlled by `src/activities/network/PicoReadWebServerActivity.h` and served by `src/network/PicoReadWebServer.h`.
