...
PicoRead runs on real hardware, so debugging usually combines local build checks and on-device logs.
...
- Whether issue reproduces after clearing `.picoread/` cache on SD card
