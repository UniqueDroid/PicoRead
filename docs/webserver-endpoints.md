...
available while PicoRead is in File Transfer or Calibre Wireless mode.
...
Examples use `picoread.local`. If mDNS does not resolve on your network, use
...
```bash
curl http://picoread.local/api/status
```
...
```bash
curl "http://picoread.local/api/files?path=/Books"
```
...
```bash
curl -OJ "http://picoread.local/download?path=/Books/MyBook.epub"
```
...
```bash
curl -X POST -F "file=@mybook.epub" "http://picoread.local/upload?path=/Books"
```
...
```bash
curl -X POST -d "name=NewFolder&path=/" http://picoread.local/mkdir
```
...
```bash
curl -X POST -d "path=/Books/old.epub&name=new.epub" http://picoread.local/rename
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub&dest=/Read" http://picoread.local/move
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub" http://picoread.local/delete
curl -X POST -d 'paths=["/Books/old.epub","/OldFolder"]' http://picoread.local/delete
```
...
```bash
curl http://picoread.local/api/settings
```
...
curl -X POST \
  ...
  http://picoread.local/api/settings
```
...
```bash
curl http://picoread.local/api/fonts
```
...
curl -X POST \
  ...
  http://picoread.local/api/fonts/upload
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"family":"Literata"}' \
  http://picoread.local/api/fonts/delete
```
...
```bash
curl http://picoread.local/api/opds
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://picoread.local/api/opds/delete
```
...
```bash
curl http://picoread.local/api/wifi
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://picoread.local/api/wifi/delete
```
...
- `picoread.local` is advertised with mDNS when available.
- `/api/status` returns `"mode": "STA"` and RSSI in dBm.

### Access Point Mode (AP)

- Device creates an open hotspot named `PicoRead-Reader`.
