...
void HalTiltSensor::update(const uint8_t mode, const uint8_t orientation, const bool inReader) {
  ...
  if ((mode != PicoReadTiltPageTurn::TILT_OFF) && !_isAwake) {
    _isAwake = wake();
    return;
  } else if ((mode == PicoReadTiltPageTurn::TILT_OFF) && _isAwake) {
    _isAwake = !deepSleep();
    return;
  }

  // If disabled, skip the rest of the polling logic and avoid unnecessary I2C traffic in non-reader activities
  if ((mode == PicoReadTiltPageTurn::TILT_OFF) || !inReader) {
    return;
  }
  ...
  float tiltAxis;
  switch (orientation) {
    case PicoReadOrientation::PORTRAIT:
      tiltAxis = mode == PicoReadTiltPageTurn::TILT_INVERTED ? -gx : gx;
      break;
    case PicoReadOrientation::INVERTED:
      tiltAxis = mode == PicoReadTiltPageTurn::TILT_INVERTED ? gx : -gx;
      break;
    case PicoReadOrientation::LANDSCAPE_CW:
      tiltAxis = mode == PicoReadTiltPageTurn::TILT_INVERTED ? gy : -gy;
      break;
    case PicoReadOrientation::LANDSCAPE_CCW:
      tiltAxis = mode == PicoReadTiltPageTurn::TILT_INVERTED ? -gy : gy;
      break;
    default:
      tiltAxis = gx;
