  ...
  if (sdFontFamilyName[0] != '\0' && registry) {
    ...
    for (int i = 0; i < static_cast<int>(families.size()); i++) {
      if (families[i].name == sdFontFamilyName) {
        return PicoReadSettings::BUILTIN_FONT_COUNT + i;
      }
    }
  }

  return fontFamily < PicoReadSettings::BUILTIN_FONT_COUNT ? fontFamily : 0;
}
}  // namespace
...
void FontSelectionActivity::onEnter() {
  ...
  fonts_.clear();
  fonts_.reserve(PicoReadSettings::BUILTIN_FONT_COUNT + (registry_ ? registry_->getFamilyCount() : 0));

  fonts_.push_back({I18N.get(StrId::STR_NOTO_SERIF), true, static_cast<uint8_t>(PicoReadSettings::NOTOSERIF)});
  fonts_.push_back({I18N.get(StrId::STR_NOTO_SANS), true, static_cast<uint8_t>(PicoReadSettings::NOTOSANS)});

  if (registry_) {
    const auto& families = registry_->getFamilies();
    for (int i = 0; i < static_cast<int>(families.size()); i++) {
      fonts_.push_back({families[i].name, false, static_cast<uint8_t>(PicoReadSettings::BUILTIN_FONT_COUNT + i)});
    }
  }
...
void FontSelectionActivity::loop() {
  ...
  if (mappedInput.wasPressed(MappedInputManager::Button::Confirm)) {
    if (selectedIndex_ == previewFontIndex_) {
      ...
      if (font.isBuiltin) {
        ...
        const int sdIdx = font.settingIndex - PicoReadSettings::BUILTIN_FONT_COUNT;
        const auto& families = registry_->getFamilies();
  ...
  buttonNavigator_.wireListNav(selectedIndex_, listSize, pageItems, [this] { requestUpdate(); });
}

void FontSelectionActivity::handleSelection() {
  const auto& font = fonts_[selectedIndex_];
  if (font.settingIndex < PicoReadSettings::BUILTIN_FONT_COUNT) {
    SETTINGS.fontFamily = font.settingIndex;
    SETTINGS.sdFontFamilyName[0] = '\0';
  } else if (registry_) {
    const int sdIdx = font.settingIndex - PicoReadSettings::BUILTIN_FONT_COUNT;
