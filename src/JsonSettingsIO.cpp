...
void applyLegacyStatusBarSettings(PicoReadSettings& settings) {
  switch (static_cast<PicoReadSettings::STATUS_BAR_MODE>(settings.statusBar)) {
    case PicoReadSettings::NONE:
      settings.statusBarChapterPageCount = 0;
      settings.statusBarBookProgressPercentage = 0;
      settings.statusBarProgressBar = PicoReadSettings::HIDE_PROGRESS;
      settings.statusBarTitle = PicoReadSettings::HIDE_TITLE;
      settings.statusBarBattery = 0;
      break;
    case PicoReadSettings::NO_PROGRESS:
      settings.statusBarChapterPageCount = 0;
      settings.statusBarBookProgressPercentage = 0;
      settings.statusBarProgressBar = PicoReadSettings::HIDE_PROGRESS;
      settings.statusBarTitle = PicoReadSettings::CHAPTER_TITLE;
      settings.statusBarBattery = 1;
      break;
    case PicoReadSettings::BOOK_PROGRESS_BAR:
      settings.statusBarChapterPageCount = 1;
      settings.statusBarBookProgressPercentage = 0;
      settings.statusBarProgressBar = PicoReadSettings::BOOK_PROGRESS;
      settings.statusBarTitle = PicoReadSettings::CHAPTER_TITLE;
      settings.statusBarBattery = 1;
      break;
    case PicoReadSettings::ONLY_BOOK_PROGRESS_BAR:
      settings.statusBarChapterPageCount = 1;
      settings.statusBarBookProgressPercentage = 0;
      settings.statusBarProgressBar = PicoReadSettings::BOOK_PROGRESS;
      settings.statusBarTitle = PicoReadSettings::HIDE_TITLE;
      settings.statusBarBattery = 0;
      break;
    case PicoReadSettings::CHAPTER_PROGRESS_BAR:
      settings.statusBarChapterPageCount = 0;
      settings.statusBarBookProgressPercentage = 1;
      settings.statusBarProgressBar = PicoReadSettings::CHAPTER_PROGRESS;
      settings.statusBarTitle = PicoReadSettings::CHAPTER_TITLE;
      settings.statusBarBattery = 1;
      break;
    case PicoReadSettings::FULL:
    default:
      settings.statusBarChapterPageCount = 1;
      settings.statusBarBookProgressPercentage = 1;
      settings.statusBarProgressBar = PicoReadSettings::HIDE_PROGRESS;
...
bool JsonSettingsIO::saveState(const PicoReadState& s, const char* path) {
  JsonDocument doc;
  doc["openEpubPath"] = s.openEpubPath;
  JsonArray recentArr = doc["recentSleepImages"].to<JsonArray>();
  for (int i = 0; i < PicoReadState::SLEEP_RECENT_COUNT; i++) recentArr.add(s.recentSleepImages[i]);
...
bool JsonSettingsIO::loadState(PicoReadState& s, const char* json) {
  JsonDocument doc;
  ...
  const int actualCount = recentArr.isNull() ? 0
                                             : std::min(static_cast<int>(recentArr.size()),
                                                        static_cast<int>(PicoReadState::SLEEP_RECENT_COUNT));
  ...
  if (s.recentSleepPos >= PicoReadState::SLEEP_RECENT_COUNT)
    s.recentSleepPos = actualCount > 0 ? s.recentSleepPos % PicoReadState::SLEEP_RECENT_COUNT : 0;
...
bool JsonSettingsIO::saveSettings(const PicoReadSettings& s, const char* path) {
  JsonDocument doc;
...
bool JsonSettingsIO::loadSettings(PicoReadSettings& s, const char* json, bool* needsResave) {
  ...
  if (doc["sleepTimeoutMinutes"].isNull() && !doc["sleepTimeout"].isNull()) {
    const uint8_t legacyValue =
        clamp(doc["sleepTimeout"] | (uint8_t)PicoReadSettings::SLEEP_10_MIN, PicoReadSettings::SLEEP_TIMEOUT_COUNT,
              (uint8_t)PicoReadSettings::SLEEP_10_MIN);
    s.sleepTimeoutMinutes = PicoReadSettings::sleepTimeoutEnumToMinutes(legacyValue);
    if (needsResave) *needsResave = true;
  }
  // Front button remap — managed by RemapFrontButtons sub-activity, not in SettingsList.
  using S = PicoReadSettings;
  s.frontButtonBack =
  ...
  PicoReadSettings::validateFrontButtonMapping(s);

  // Font family — uses dynamic getter/setter in SettingsList so the generic loop skips it.
  const uint8_t storedFontFamily = doc["fontFamily"] | (uint8_t)0;
  s.fontFamily = clamp(storedFontFamily, PicoReadSettings::BUILTIN_FONT_COUNT, 0);
  // SD card font family name — not in SettingsList, load manually
  const char* sfn = doc["sdFontFamilyName"] | "";
  strncpy(s.sdFontFamilyName, sfn, sizeof(s.sdFontFamilyName) - 1);
  s.sdFontFamilyName[sizeof(s.sdFontFamilyName) - 1] = '\0';
  if (storedFontFamily == PicoReadSettings::LEGACY_OPENDYSLEXIC && s.sdFontFamilyName[0] == '\0') {
    s.fontFamily = PicoReadSettings::NOTOSERIF;
    strncpy(s.sdFontFamilyName, "OpenDyslexic", sizeof(s.sdFontFamilyName) - 1);
    s.sdFontFamilyName[sizeof(s.sdFontFamilyName) - 1] = '\0';
    if (needsResave) *needsResave = true;
  } else if (storedFontFamily >= PicoReadSettings::BUILTIN_FONT_COUNT) {
    if (needsResave) *needsResave = true;
  }
