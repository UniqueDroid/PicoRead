...
#include "components/icons/text32.h"
...
void PicoReadXtTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
...
// (see PicoReadXtMetrics::homeRecentBooksCount). Title/author/"Continue Reading"
...
void PicoReadXtTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
                                          const int selectorIndex, bool& coverRendered, bool& coverBufferStored,
                                          bool& bufferRestored, std::function<bool()> storeCoverBuffer) const {
  const int count = PicoReadXtMetrics::values.homeRecentBooksCount;
  const int tileWidth = (rect.width - 2 * PicoReadXtMetrics::values.contentSidePadding) / count;
  const int tileY = rect.y;
  const int tileHeight = PicoReadXtMetrics::values.homeCoverTileHeight;
  ...
  if (!coverRendered) {
    for (int i = 0; i < shown; i++) {
      ...
      const int tileX = PicoReadXtMetrics::values.contentSidePadding + tileWidth * i;
      bool hasCover = false;
      if (!coverPath.empty()) {
...
void PicoReadXtTheme::drawButtonHints(GfxRenderer& renderer, const char* btn1, const char* btn2, const char* btn3,
